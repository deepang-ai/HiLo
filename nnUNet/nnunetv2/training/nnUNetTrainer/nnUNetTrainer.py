...
class nnUNetTrainer(object):
    def __init__(self, loss: str, optimizer: str, lr: float, num_epochs: int, batch_size: int, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for i, k in enumerate(dataset_val.keys()):
                ...
                if hasattr(self, 'configure_inference_for_case'):
                    self.configure_inference_for_case(predictor, k, properties)
                try:
                    prediction = predictor.predict_sliding_window_return_logits(data)
                finally:
                    if hasattr(self, 'clear_inference_for_case'):
                        self.clear_inference_for_case(predictor)
